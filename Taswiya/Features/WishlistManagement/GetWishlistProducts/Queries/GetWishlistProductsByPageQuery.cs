...
{
    public record GetWishlistProductsByPageQuery(string CustomerId, PaginationHelper PaginationParams) 
        : IRequest<RequestResult<IReadOnlyList<GetWishlistProductsResponseViewModel>>>;
    
    public class GetWishlistProductsByPageQueryHandler(IRepository<WishlistItem> repository) 
        : IRequestHandler<GetWishlistProductsByPageQuery, RequestResult<IReadOnlyList<GetWishlistProductsResponseViewModel>>>
    {
        private readonly IRepository<WishlistItem> _repository = repository;

        public async Task<RequestResult<IReadOnlyList<GetWishlistProductsResponseViewModel>>> Handle(GetWishlistProductsByPageQuery request, CancellationToken cancellationToken)
        {
            var wishlistProducts = await _repository.GetByPage(w => w.CustomerId == request.CustomerId, request.PaginationParams)
                .Include(w => w.Product)
